...
from SonGokuRobot import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, dispatcher)
from SonGokuRobot.modules.disable import DisableAbleCommandHandler
from SonGokuRobot.modules.helper_funcs.chat_status import (
    bot_admin, can_restrict, connection_status, is_user_admin,
    is_user_ban_protected, is_user_in_chat, user_admin, user_can_ban)
from SonGokuRobot.modules.helper_funcs.extraction import extract_user_and_text
from SonGokuRobot.modules.helper_funcs.string_handling import extract_time
from SonGokuRobot.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Ah wait, WTF...You baka?")
        return log_message

    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Not gonna ban Goku! He created me bruh.")
            return log_message
        elif user_id in DEV_USERS:
            message.reply_text("I can't kill Whis!")
            return log_message
        elif user_id in DRAGONS:
            message.reply_text(
                "No!, Beerus sama is innocent. YOU WILL GET BANNED BY GOKU FOR YOUR BAD INTENSIONS.")
            return log_message
        elif user_id in DEMONS:
            message.reply_text(
                "Can't go against Gohan! He is innocent."
            )
            return log_message
        elif user_id in TIGERS:
            message.reply_text(
                "Wait.. Goku is coming...he will deal with you."
            )
            return log_message
        elif user_id in WOLVES:
            message.reply_text("AAAAAAAAAAAAAAAAAAAAAAA! Can't ban wolves!")
            return log_message
        else:
    ...
    try:
        ...
        reply = (
            f"<code>❕</code><b>FINAL... Ka..Me..Ha..Me..Haa!!\n BAN EVENT</b>\n"
