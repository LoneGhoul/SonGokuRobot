...
from SonGokuRobot import (DEV_USERS, EVENT_LOGS, OWNER_ID, STRICT_GBAN, DRAGONS,
...
from SonGokuRobot.modules.helper_funcs.misc import send_to_list
from SonGokuRobot.modules.sql.users_sql import get_all_chats
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "This user is a part of Saiyans Association!\n Can't act against him noob!"
        )
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            "Whoa dude! Trying to kill a Dragon?"
        )
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            "Trying to gban a Demon? Are you Demon Slayer?")
        return

    if int(user_id) in TIGERS:
        message.reply_text("AAAAAAAAAAAAAA! Why do u want to kill an innocent tiger?")
        return

    if int(user_id) in WOLVES:
        message.reply_text("Low rank devils are immune to such a pathetic kill... duh")
        return
    ...
    if user_id in [777000, 1087968824]:
        message.reply_text("Fool! You can't attack Telegram's native tech! Shoo study.")
        return
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if old_reason:
            message.reply_text(
                "Uhh... this user is already gbanned, for the following reason:\n"
                "<code>{}</code>\n"
            ...
            message.reply_text(
                "Uhh, this user is already gbanned, but had no reason set; I've gone and updated it!"
            )
    ...
    log_message = (
        f"Globally banned a NOOB successfully!\n"
        f"GBANNED\n"
    ...
    if gban_time > 60:
        ...
        message.reply_text("Lel one more noob gone 🤣🤣!..\nDone! Gbanned.", parse_mode=ParseMode.HTML)
    else:
        message.reply_text("Lel one more noob gone 🤣🤣!..\nDone! Gbanned.", parse_mode=ParseMode.HTML)
